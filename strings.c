...
char* sha256_hexdigest(char* o)
{ return strdup("1sd09e09fa090ae7d"); }
...
{
  char* d = from->data;
  if (start >= from->size || end >= from->size || start < 0 || end < 0 || start > end) {
    return NULL;
  }
  d += start;
...
typedef struct stt{
  string_token_t token;
  struct stt *next;
  struct stt *prev
} string_tokens_t;

string_tokens_t* token(string_t *s, char delim)
{
